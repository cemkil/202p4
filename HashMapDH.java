...
public class HashMapDH<Key, Value> extends AbstractHashMap<Key, Value> {
  ...
  protected int primaryHash(int hashCode) {
    ...
    return Math.abs(hashCode * a + b) % P ;
  }
  ...
  public int hashValue(Key key, int iter) {
    ...
    int first = primaryHash(k);
    int second = secondaryHash(k);
    int a = Math.abs(first+ iter * second) % N;
  return a;
  }
  ...
  protected void checkAndResize() {
    if (loadFactor() > criticalLoadFactor) {
      ...
      keyList.clear();
      n=0;
      for(HashEntry<Key,Value> bucket : temp){
        if(bucket != null){
  ...
  public Value put(Key k, Value v) {
    ...
    while(true){
      if(buckets[hashValue(k,i)]==null){
        ...
        Value temp = buckets[hashValue(k,i)].getValue();
        buckets[hashValue(k,i)].setValue(v);
        return temp;
      }else if(buckets[hashValue(k,i)].getKey().equals("DEFUNCT")){
        buckets[hashValue(k,i)].setKey(k);
        buckets[hashValue(k,i)].setValue(v);
        keyList.add(k);
        n++;
        return null;
  ...
  public Value remove(Key k) {
    ...
    while(true){
      if(buckets[hashValue(k,i)]==null){
        ...
        Value temp = buckets[hashValue(k,i)].getValue();
        buckets[hashValue(k,i)].setKey((Key) "DEFUNCT");
        keyList.remove(k);
        n--;
        return temp;
      }
