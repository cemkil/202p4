...
public class HashBasedWordCO {
  ...
  public HashSet<String> getUniqueWords(String text) {
    ...
    for (String sentence : sentences) {
      for (String word : sentence.split("\\s+")) {
        ...
        // the minWordLen
        if(! isIgnored(word) && word.length() >= minWordLen){
  ...
  public void fillCoMat(String text, String[] keyWords) {
    ...
    for(String sentence : sentences){
      String[] txtWords = sentence.split("\\s+");
      for(int i = 0 ; i<txtWords.length;i++) {
        if (txtWords[i].length() >= minWordLen && !isIgnored(txtWords[i])) {
          txtWords[i] = txtWords[i].toLowerCase();
          if (isNull) {
            for (int j = i - windowSize; j <= i + windowSize; j++) {
              try{
                if(txtWords[j].length()>= minWordLen && !isIgnored(txtWords[j]) && j!=i) {
                  coMat.get(txtWords[i]).increment(txtWords[j]);
                }
            ...
            if (CheckInArray(txtWords[i], keyWords)) {
              for (int j = i - windowSize; j <= i + windowSize; j++) {
                try{
                  if(txtWords[j].length()>= minWordLen && !isIgnored(txtWords[j]) && j!=i) {
                    coMat.get(txtWords[i]).increment(txtWords[j]);
  ...
  public boolean CheckInArray(String s , String[] arr){
    for(String ar : arr){
      if(s.equals(ar)){
        return true;
      }
